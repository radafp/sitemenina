...
require_once ADMIN_PATH."_enquetes/inc/topo-enquetes-lista.php";
?>
<script>
...
<div class="divTableLista clear">
    <div class="divTr head">
        <div class="divTd">&nbsp;</div>
        <div class="divTd">Data Cadastro</div>
        <div class="divTd">Enquete</div>
        <div class="divTd">Mostrar</div>
    </div>
    <?
    $regiao = isset($_SESSION[ADMIN_SESSION_NAME.'_regiao']) ? $_SESSION[ADMIN_SESSION_NAME.'_regiao'] : '';
    
    // $q = mysql_query("SELECT * FROM enquetesPerguntas WHERE regiao = '{$regiao}' ORDER BY dataCadastro DESC", $conexao);
    // $n = mysql_num_rows($q);
    ...
    $limit_por_pag = 30;
    $q = mysql_query("SELECT * FROM enquetesPerguntas WHERE regiao = '{$regiao}' ORDER BY dataCadastro DESC LIMIT $pag, $limit_por_pag", $conexao);
    
    $rows = mysql_query("SELECT * FROM enquetesPerguntas WHERE regiao = '{$regiao}'", $conexao);
    ...
    if ($count_registros>0)
    {
        ...
        ?>
            <div class="divTr">
                ...
                <div class="divTd">
                    <a href="http://<?=ADMIN_URL;?>/principal.php?id=<?=$id;?>&subid=3&cod=<?=$tp['cod'];?>"><?=dataBr($tp['dataCadastro']);?></a>
                </div>
                <div class="divTd">
                    <a href="http://<?=ADMIN_URL;?>/principal.php?id=<?=$id;?>&subid=3&cod=<?=$tp['cod'];?>"><?=$tp['pergunta'];?></a>
                </div>
                <div class="divTd">
