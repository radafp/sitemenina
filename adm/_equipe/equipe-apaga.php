...
if($cods != '')
{
    ...
    {
        /** EXCLUIR ARQUIVOS - */
        $q = mysql_query("SELECT * FROM equipe WHERE cod = '{$cod}'");
        while($tp = mysql_fetch_assoc($q))
        {
            $qArquivos = mysql_query("SELECT * FROM arquivos WHERE codReferencia = '{$tp['cod']}' AND referencia = 'equipe'");
            while($tpArquivos = mysql_fetch_assoc($qArquivos))
            {
                for($a=0;$a<5;$a++)
                {
                    $unlink = @unlink(PROJECT_PATH."arquivos/equipe/".$tpArquivos['arquivo']);
                    if($unlink)
                    {
                        break;
                    }
            ...
            $sqlDel = "DELETE FROM equipe WHERE cod = '{$tp['cod']}'";
            for($a=0;$a<5;$a++)
            {
