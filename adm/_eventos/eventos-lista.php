...
require_once ADMIN_PATH."_eventos/inc/topo-eventos-lista.php";
?>
<script>
$(document).ready(function()
{
    $(".mostrar").click(function()
    {
        ...
        $.ajax(
        {
            type: "POST",
            async: false,
            url: "http://"+ADMIN_URL+"/_eventos/ajax/ajaxMostrarLista.php", //URL de destino
            data:
            {
                cod: _cod,
                mostrar: _mostrar
...
<div class="divTableLista clear">
    ...
    // $q = mysql_query("SELECT * FROM eventos WHERE regiao = '{$regiao}' ORDER BY dataInicio DESC", $conexao);
    // $n = mysql_num_rows($q);
    ...
    $limit_por_pag = 30;
    $q = mysql_query("SELECT * FROM eventos WHERE regiao = '{$regiao}' ORDER BY ordem ASC LIMIT $pag, $limit_por_pag", $conexao);
    
    $rows = mysql_query("SELECT * FROM eventos WHERE regiao = '{$regiao}'", $conexao);
    ...
    if ($count_registros>0)
    {
    	while($tp = mysql_fetch_assoc($q))
    	{
            ...
            $qFotos = mysql_query("SELECT cod FROM arquivos WHERE codReferencia = '{$tp['cod']}' AND tipo = '2' AND referencia = 'eventos'");
