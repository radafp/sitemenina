...
$(document).ready(function()
{
    ...
    $("#salvar").click(function()
    {
        ...
        $.ajax(
        {
            type: "POST",
            async: false,
            url: "http://"+ADMIN_URL+"/_artistas/ajax/ajaxOrdenaFotos.php", //URL de destino
            data:
            {
                ordem : ordem,
...
<div class="divTableLista clear drag" style="margin-top: 0;">
    <?
    $q = mysql_query("SELECT * FROM arquivos WHERE codReferencia = '$cod' AND tipo = '1' AND referencia = 'artistas' ORDER BY ordem ASC", $conexao);
    //echo mysql_error();
    $n = mysql_num_rows($q);
    if($n>0)
    {
    	...
    	?>
            <div class="divTr" style="cursor: all-scroll;" data-cod="<?=$tp['codigo'];?>">
                ...
                <div class="divTd">
                    <img src="<?=ssl().PROJECT_URL.'/arquivos/artistas/'.$tp['arquivo'];?>" title="<?=$tp['legenda'];?>" />
                </div>
                <div class="divTd">
                    <?=$tp['capa'] == 1 ? 'SIM' : 'NÃO';?>
                </div>
